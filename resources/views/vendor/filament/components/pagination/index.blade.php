        ...
        <span class="fi-pagination-overview">
            {{
                trans_choice(
                    ...
                    $paginator->total(),
                    [
                        'first' => $nf($paginator->firstItem() ?? 0),
                        'last' => $nf($paginator->lastItem() ?? 0),
                        'total' => $nf($paginator->total()),
                    ],
                )
            }}
        </span>
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item
                            ...
                            :aria-label="trans_choice('filament::components/pagination.actions.go_to_page.label', $page, ['page' => $nf($page)])"
                            :label="$nf($page)"
